...
train_dataset = FashionMNIST('./fashionmnist_data/', train=True, download=True,
                             transform=transforms.Compose([
                                 transforms.ToTensor()
                             ])) + FashionMNIST('./fashionmnist_data/', train=False, download=True,
                                                transform=transforms.Compose([
                                                    transforms.ToTensor()
                                                ]))
...
hgl = HGL(dim=d, units=30, init_mode=1)  # 5,30效果最好
...
def val_epoch(encoder, decoder, device, dataloader):
    ...
    with torch.no_grad():
        ...
        print('===' * 10)
        display(confusion, dataset='fashion_mnist')
